...
export const papersChart = async (scene) => {
  ...
  let scaleX = d3.scaleSymlog().domain(d3.extent(data, d => d["Downloads_Xplore"])).range([0, 10]).constant(5000).nice();   //Downloads
  let scaleY = d3.scaleSymlog().domain(d3.extent(data, d => d["AminerCitationCount"])).range([0, 5]).constant(100).nice();  //Citations
  let scaleZ = d3.scaleBand().domain(data.map(d => d.Year)).range([0, 10]).padding(0.1);  //Year
  ...
  let dots = chart.bindInstance(rootDot, data)
                  .name('scatter-dots')
